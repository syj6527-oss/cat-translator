...
function cleanResult(text) {
    if (!text) return "";
    return text.replace(/\[Alternative to:.*?\]/gs, "").replace(/\[Note:.*?\]/gs, "").replace(/\[CRITICAL:.*?\]/gs, "").trim();
}
...
async function fetchTranslation(text, isInput = false, previousTranslation = null) {
    ...
    if (!previousTranslation && translationCache[cacheKey]) {
        toastr.info("🐱 캐시 사용: 토큰 아꼈다!");
        return translationCache[cacheKey];
    }

    const targetLang = isInput ? "English" : settings.targetLang;
    const basePrompt = isInput ? "Translate to natural English. Preserve format." : settings.prompt.replace('{{language}}', targetLang);
    const cleanedPrev = cleanResult(previousTranslation);
    const variationPrompt = cleanedPrev ? `\n\n[Note: Use DIFFERENT phrasing than: "${cleanedPrev}"]` : "";
...
function injectInputButtons() {
    ...
    if (!target) return;
    if (target.prev('#cat-input-btn').length > 0) return; // 이미 있으면 절대 건드리지 않음

    // 혹시 다른 곳에 생성된 유령 버튼이 있다면 한 번만 청소
                ...
                <div class="inline-drawer-content" style="display: none;">
                    ...
                    <div id="direct-mode-settings" style="border-left: 2px solid #a8c7fa; padding-left: 10px; margin-bottom: 15px; display: ${settings.profile === '' ? 'block' : 'none'};">
                        ...
                        <div class="cat-setting-row"><label>Model</label><select id="ct-model" class="text_pole">
                            <option value="gemini-1.5-flash">Gemini 1.5 Flash</option><option value="gemini-1.5-pro">Gemini 1.5 Pro</option><option value="gemini-2.0-flash">Gemini 2.0 Flash</option><option value="gemini-2.0-pro-exp-02-05">Gemini 2.0 Pro Exp</option>
                        </select></div>
                    </div>
                    ...
                    <div class="cat-setting-row"><label>번역 프롬프트</label><textarea id="ct-prompt" class="text_pole" rows="4"></textarea></div>
                    <div class="cat-setting-row" style="margin-top: 15px;"><button id="cat-save-btn" class="menu_button">설정 저장 🐱</button></div>
                    <div style="font-size: 0.8em; opacity: 0.1; text-align: center; margin-top: 5px;">v6.5.0 Safe Build</div>
                </div>
            </div>`;
...
jQuery(() => {
    ...
    // 💡 [v6.5.0] 정밀 타격 감시: 하단바 영역만 감시해서 성능 부하 차단
    const container = document.querySelector('#send_form, #send_container, #footer');
    if (container) {
        const observer = new MutationObserver(() => { injectInputButtons(); });
        observer.observe(container, { childList: true, subtree: true });
    }

    // 💡 보험용 체크: 1초에 한 번만 (성능 최적화)
    setInterval(injectInputButtons, 1000);
    ...
    $(document).on('mouseenter touchstart', '.mes', function() {
        if (!$(this).find('.cat-btn-group').length) {
            ...
            const transBtn = $('<div class="cat-mes-trans-btn" title="번역" style="cursor:pointer; opacity:0.6; font-size:1.6em; line-height:1;"><span class="cat-emoji-icon">🐱</span></div>');
            const revertBtn = $('<div class="cat-mes-revert-btn fa-solid fa-rotate-left" title="복구" style="cursor:pointer; color:#ffb4a2; opacity:0.6; font-size:1.1em;"></div>');
